						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>可约人数</label>
										<select id="keyuerenshuSelect" name="keyuerenshu" class="form-control">
												<option value=""></option>
												<option class="keyuerenshuOption" value="1">
													1
												</option>
										</select>	
									</div>	
									...
									<div class="form-group col-md-6">
										<label>预约说明</label>
										<input name="yuyueshuoming" id="yuyueshuoming" name="yuyueshuoming" class="form-control" placeholder="预约说明">
									</div>
									<div class="form-group col-md-6">
										<label>账号</label>
	...
	<script>
		...
		function submit() {
			...
			if(validform() ==true && compare() == true){
				if(crossFlag) {
					if(Number(keyuerenshuKucun) < Number($('#keyuerenshuSelect').val())){
						alert('可约人数不能大于'+keyuerenshuKucun+'！请重新输入');
						return false;
					}
				}
				let data = {};
		...
		function dataBind(){
			...
			var keyuerenshuOptions = document.getElementsByClassName("keyuerenshuOption");
			for(var keyuerenshuCount = 0; keyuerenshuCount < keyuerenshuOptions.length;keyuerenshuCount++){
				if(keyuerenshuOptions[keyuerenshuCount].getAttribute('value') == ruleForm.keyuerenshu){
					keyuerenshuOptions[keyuerenshuCount].setAttribute('selected','selected');
				}
			}
			$("#yuyueshijian-input").val(ruleForm.yuyueshijian);
			$("#yuyueshuoming").val(ruleForm.yuyueshuoming);	
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.keyuerenshu != null && res.data.keyuerenshu != ''){
						...
						keyuerenshuKucun = res.data.keyuerenshu;
					...
					if(res.data != null && res.data != '' && res.data.yuyueshuoming != null && res.data.yuyueshuoming != ''){

						$("#yuyueshuoming").val(res.data.yuyueshuoming);
						$("#yuyueshuoming").attr('readonly','readonly');
					}
		...
		function crossTableUpdate(){
			...
			if(flag){
				httpJson(crossTableName + "/update","POST",{
					id: crossTableId,
					keyuerenshu: Number(crossRuleForm.keyuerenshu) - Number($('#keyuerenshuSelect').val()),
				},(res)=>{
					if(res.code == 0){
						console.log('跨表更新成功');
						return true;
		...
		function crossTableUpdateValidation(){
			//防止减法导致库存负值
			if(Number(keyuerenshuKucun) < Number($('#keyuerenshu').val())){
				alert('可约人数不能大于'+keyuerenshuKucun+'！请重新输入');
				return false;
			}
			return true;
