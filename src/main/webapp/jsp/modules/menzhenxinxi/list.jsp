...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">门诊信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>门诊信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">门诊信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										科室名称
									</label>
									<input type="text" id="keshimingchengSearch" class="form-control form-control-sm" placeholder="请输入科室名称" aria-controls="tableId">
									<label>
										科室类型
									</label>
									<input type="text" id="keshileixingSearch" class="form-control form-control-sm" placeholder="请输入科室类型" aria-controls="tableId">
									<label>
										医生姓名
									</label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('keshileixing')">科室类型<i id="keshileixingIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('shangbanriqi')">上班日期<i id="shangbanriqiIcon" class="fa fa-sort"></i></th>
											<th>科室图片</th>
											<th>操作</th>
										</tr>
									</thead>
	...
	<script>
		...
		function search(){
			...
			if($('#keshileixingSearch').val() != null && $('#keshileixingSearch').val() != ''){
				searchForm.keshileixing ="%" +  $('#keshileixingSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("menzhenxinxi/page","GET",{
				...
				keshileixing : searchForm.keshileixing,	
		...
		function setDataRow(item,number){
			...
			var keshileixingCell  = document.createElement('td');

						 keshileixingCell.innerHTML = item.keshileixing;

			row.appendChild(keshileixingCell);
			...
			var shangbanriqiCell  = document.createElement('td');

						  shangbanriqiCell.innerHTML = item.shangbanriqi;


			row.appendChild(shangbanriqiCell);
			var paibanbiaoCell  = document.createElement('td');

			
			var yishengjieshaoCell  = document.createElement('td');

			
			var keshitupianCell  = document.createElement('td');

				                var keshitupianImg = document.createElement('img');
			keshitupianImg.setAttribute('src',baseUrl+item.keshitupian);
			keshitupianImg.setAttribute('height',100);
			keshitupianImg.setAttribute('width',100);
			keshitupianCell.appendChild(keshitupianImg);


			row.appendChild(keshitupianCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "预约挂号"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'yuyueguahao','','[1]','已预约')");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("menzhenxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussmenzhenxinxi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"menzhenxinxi");
